...
class Experiment1:
    ...
    def is_enemy_ancient_alive(self):
        ea = self.world.find_entity_by_name(
            "dota_badguys_fort").getOrigin()
        if not ea:
            print("---")
            print("TEST PASSED:")
            print("Enemy ancient has been destroyed and match is finished")
            print("---")

    def actions(self, hero):
        self.is_enemy_ancient_alive()
